"""
Training script for Streaming Flow Policy (Stochastic) on lasso task.
Follows pusht/experiments/sfps_obs/train.py patterns.

Usage:
    python -m streaming_flow_policy.lasso.experiments.sfps.train --zarr_path /path/to/data.zarr
"""
import argparse
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train SFPS on lasso task')
    parser.add_argument('--zarr_path', type=str, required=True,
                        help='Path to zarr dataset')
    parser.add_argument('--save_path', type=str, default='models/lasso_sfps.pth',
...
def main():
    ...
    # Create velocity network with fc_timesteps=2 for dual-path flow
    velocity_net = ConditionalUnet1D(
        input_dim=action_dim,
        global_cond_dim=obs_dim * obs_horizon,
        fc_timesteps=2,  # Dual path for stochastic (action + latent)
    )
    ...
    # Create policy
    policy = StreamingFlowPolicyStochasticLasso(
    ...
    print(f"Training SFPS on lasso task")
    print(f"  Dataset size: {len(dataset)}")
    print(f"  Batch size: {args.batch_size}")
    print(f"  Epochs: {args.num_epochs}")
    print(f"  σ0: {args.sigma0}, σ1: {args.sigma1}")
